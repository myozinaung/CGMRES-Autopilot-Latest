...
void simulation(NMPCSolver& nmpc, const double* initial_state_vec, 
  ...
  std::ofstream state_data(savefile_header + "_state.csv"), 
    control_input_data(savefile_header + "_control_input.csv"), 
    error_data(savefile_header + "_error.csv"),
  ...
  for (double current_time=start_time; current_time<end_time; 
    ...
    std::cout << current_time << ","
        ...
        << control_input_vec[0] << "," << nmpc.getErrorNorm(current_time, current_state_vec) << std::endl;
  ...
  std::cout << "End simulation\n" 
      ...
      << total_time/((int)( (end_time-start_time)/(sampling_period)))*1000 
      << " [mili-sec]" << std::endl;
